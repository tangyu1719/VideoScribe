...
class BGEEmbeddingModel:
    ...
    def _load_model(self):
        ...
        model_sources = [
            ...
            "sentence-transformers/all-MiniLM-L6-v2",  # 备选英文模型
        ]
        
        for model_source in model_sources:
            try:
                logger.info(f"正在加载模型: {model_source}")
                self.model = SentenceTransformer(model_source)
                self.dimension = self.model.get_sentence_embedding_dimension()
                logger.info(f"模型加载成功，维度: {self.dimension}")
                return
            except Exception as e:
                logger.warning(f"加载模型 {model_source} 失败: {e}")
                continue
        
        logger.error("所有模型源都加载失败")
        self.model = None
...
class HybridRetriever:
    ...
    def __init__(self, vector_store: MilvusVectorStore, bm25_retriever: BM25Retriever):
