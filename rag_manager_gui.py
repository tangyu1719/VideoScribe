...
# 尝试导入新的知识库管理器（服务启动时初始化）
try:
    from kb_manager import get_knowledge_base, KnowledgeBaseManager
...
class RAGManagerGUI:
    ...
    def _create_ui(self):
        ...
        # 创建 Treeview
        columns = ("序号", "文件名", "路径", "类型", "大小", "块数", "添加时间")
        ...
        self.file_tree.column("文件名", width=200, anchor="w")
        self.file_tree.column("路径", width=250, anchor="w")
        self.file_tree.column("类型", width=80, anchor="center")
        self.file_tree.column("大小", width=80, anchor="center")
        self.file_tree.column("块数", width=80, anchor="center")
        self.file_tree.column("添加时间", width=150, anchor="center")
    ...
    def _update_file_tree(self):
        ...
        for i, record in enumerate(self.file_records, 1):
            self.file_tree.insert(
                "",
                tk.END,
                iid=record.get('file_path', str(i)),
                values=(
                    i,
                    os.path.basename(record.get('file_path', '未知')),
                    record.get('file_path', '未知'),
    ...
    def _process_file_async(self, file_path):
        """异步处理文件添加（真正向量化）"""
                ...
                if KB_MANAGER_AVAILABLE:
                    try:
                        ...
                        if success:
                            ...
                            # 更新文件记录
                            self._update_file_record(file_path, file_size, actual_chunks)
        ...
        # 启动检查结果队列
        self._check_process_result(result_queue)
    ...
    def _check_process_result(self, result_queue):
        """检查文件处理结果（线程安全）"""
        try:
            # 非阻塞方式获取结果
            ...
            if status == 'success':
                self._on_process_complete(True, file_path, message)
            else:
                self._on_process_complete(False, file_path, message)
                
        except queue.Empty:
            # 队列为空，100ms后再次检查
            self.window.after(100, lambda: self._check_process_result(result_queue))
        except Exception as e:
            print(f"检查结果时出错: {e}")
    
    def _on_process_complete(self, success, file_path, message=""):
        """处理完成回调"""
        self.progress_bar.stop()
        self.progress_label.config(text="就绪")
        ...
        if success:
            ...
            self._refresh_stats_from_records()
            messagebox.showinfo(
                "成功",
